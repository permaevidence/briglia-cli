...
| `briglia menu` | the main way in: setup and settings in the browser (English or Italian). Pick how Briglia thinks — ChatGPT subscription (default, easiest), OpenCode Go, OpenRouter or a local model (LM Studio/Ollama, models listed from the server) — then Telegram (the chat ID is detected), Serper and Jina, the OpenAI key (optional with ChatGPT, required otherwise: web research runs on it), AgentMail email, Full Disk Access / keep-awake and the document/media toolchain. Later it is the settings hub: switch or add providers, model and thinking level, keys, and Start / Stop Briglia (Stop on Linux also turns off start-at-boot). Each key is checked and saved the moment it is pasted. Same local server and link/cookie protection as `briglia quicksetup`; while it is open Briglia is paused |
| `briglia quicksetup` | the previous browser Quick Setup (kept as a fallback) on a new installation; provider, model, ChatGPT account and tool-key settings on an existing installation |
