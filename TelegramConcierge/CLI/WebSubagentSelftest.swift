...
struct WebSubagentSelftest: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        var subagentsFlag: Bool? = true
        var webFlag: Bool? = false
        AvailableTools.subagentsStoredFlagOverrideForTesting = { subagentsFlag }
        AvailableTools.webSubagentStoredFlagOverrideForTesting = { webFlag }
        defer {
        ...
        do {
            ...
            subagentsFlag = false; webFlag = true
            ...
            subagentsFlag = true; webFlag = false
        }
        ...
        do {
            ...
            for index in 0..<(SubagentSessionRegistry.maxSessions + 5 - generalBefore) {
                clock = at(2026, 4, 10, 12, 0, 0).addingTimeInterval(Double(index))
        ...
        do {
            ...
            let handle = await SubagentBackgroundRegistry.shared.spawn(invocation: invocation, sessionId: webSessionId, parentTools: AvailableTools.all(includeWebSearch: true),
                                                                         openRouterService: service, toolExecutor: executor, imagesDirectory: images, documentsDirectory: documents)
            let foreground = await SubagentRunner().run(invocation: invocation, sessionId: webSessionId, openRouterService: service, toolExecutor: await executor.makeChildExecutor(),
                                                        imagesDirectory: images, documentsDirectory: documents, parentTools: AvailableTools.all(includeWebSearch: true))
            ...
            check("9.1 Agent(session_id, run_in_background) resumes the session (handle carries it); a concurrent foreground resume serializes on the FIFO lock; both commit",
                  handle.sessionId == webSessionId && completions.count == 1 && completions[0].handle.sessionId == webSessionId
                  && completions[0].result.sessionId == webSessionId && completions[0].result.isNewSession == false && completions[0].result.error == nil
        ...
        do {
            ...
            webFlag = true; subagentsFlag = true
        ...
        do {
            ...
            check("11.3 switch off restores every inventory to its baseline bytes",
                  { webFlag = false; defer { webFlag = true }
